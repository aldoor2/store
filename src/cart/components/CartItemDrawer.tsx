...
const CartItemDrawer: React.FC<Props> = ({
  item,
  onClose,
  onSubmit,
  ...props
}) => {
  const [formData, setFormData] = React.useState<CartItem>(() => ({
    ...item,
    options: {},
  }));
  ...
  const options = React.useMemo(
    () =>
      Object.entries(item.options).map(([title, options]) => ({
        title,
        options,
      })),
    [item]
      ...
      <DrawerOverlay>
        <DrawerContent paddingTop={4}>
          <DrawerHeader paddingX={4}>
            ...
            >
              <Text fontSize={{ base: "2x1", sm: "3x1" }}>{item.title}</Text>
                      ...
                      <List spacing={2}>
                        {category.options.map((option) => (
                          <ListItem
                            key={option.title}
                            borderColor={
                              formData.options?.[option.category]?.[0]
                                ?.title === option.title
                                ? "primary.500"
                                : "transparent"
                            }
